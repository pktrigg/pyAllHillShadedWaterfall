...
# Contact
# Sam Johnstone
# Ph.D. Candidate - Geomorphology - Stanford University

# Goodbye ArcGIS, hello Python - part 3 - Slope and hillshades
...
# filename = '/this/is/the/path/floatn47w119_13.flt'
# data = gdal.Open(filename, GA_ReadOnly)

# dx = 10 #grid spacing
# utmZone = 10  #negative for southern hemisphere

# projData = convertToUTM(data, dx, utmZone)

# #Read the projected data as a numpy array, flip it to (my) expected orientation
# gridNew = np.flipud(projData.ReadAsArray().astype(np.float))

# #Create arrays of the x and y coordinates of each pixel (the axes)
# xcoordinates,ycoordinates = getDEMcoords(projData)

# #Data range
# dataExtent = [np.amin(xcoordinates), np.amax(xcoordinates),...
#               np.amax(ycoordinates), np.amin(ycoordinates)]

# #Create 2 2d grids describing x and y coordinates
# X,Y = np.meshgrid(xcoordinates, ycoordinates)

# #Create hillshade
# hs = calcHillshade(gridNew, dx, 315, 45)

# #Calculate slope
# Sx,Sy = calcFiniteSlopes(gridNew,dx)
# Smag = np.sqrt(Sx**2 + Sy**2)

# f2 = plt.imshow(Smag, interpolation = 'bilinear',...
#         cmap='coolwarm', vmin=0, vmax=1, extent=dataExtent)
# f1 = plt.imshow(hs, interpolation = 'bilinear',...
#         cmap='gray', alpha = 0.5, extent=dataExtent)
# plt.gca().invert_yaxis()
# plt.show()

# data = None
# projData = None
# Putting this all together with stuff from previous parts, we can import our dataset, project it, calculate slope and our hillshade, and then draw the slope (with the hillshade overlain at 50% opacity). Not to shabby.
